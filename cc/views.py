...
def HomePageView(request):
    ...
    if request.user.is_authenticated:
        ...
        user_is_member = UserProfile.objects.get(user=current_user)
        if user_is_member.joined_in_community == False:
            return HttpResponseRedirect(reverse('cc:CommunityRequestView'))
        else:
            HttpResponseRedirect(reverse('cc:homepage'))
    ...
    if request.method == 'POST':
        ...
        if user:
            if user.is_active:
                ...
                id = current_user.id
                user_profile_active = UserProfile.objects.get(
                    user=current_user)
                if user_profile_active.active_account == False:
                    return HttpResponseRedirect(reverse('cc:otp_page', kwargs={'id': id}))
                # if user_profile_active.joined_in_community == False:
                #     return HttpResponseRedirect(reverse('cc:CommunityRequestView'))
...
def MyApprovalsView(request, id):
    ...
    my_requests = CommunityRequests.objects.all().filter(
        Community_Name=community).order_by('-timestamp')
...
def acceptjoin(request, id, community_id, member):
    ...
    community_add.people.add(member), UserProfile.objects.all().filter(
        user=member).update(joined_in_community=True)
...
def RegisterView(request):
    if request.method == "POST":
        profile_pic = request.POST.get('q31_uploadProfile')
        first_name = request.POST.get('q24_name[first]')
        last_name = request.POST.get('q24_name[last]')
        mobile_number = request.POST.get('q9_mobileNumber[full]')
        email = request.POST.get('q11_email11')
        user_name = request.POST.get('q33_username')
        password = request.POST.get('q34_password')
        full_address = request.POST.get('q7_address7[addr_line2]')
        city = request.POST.get('q7_address7[city]')
        state = request.POST.get('q7_address7[state]')
        pincode = request.POST.get('q7_address7[postal]')
        country = request.POST.get('q7_address7[country]')

        new_user = User.objects.create(username=user_name, password=make_password(password), email=email, first_name=first_name,
                                       last_name=last_name)
        new_user.save()

        new_user_profile = UserProfile(
            user=new_user, profile_pic=profile_pic, mobileNumber=mobile_number,
            city=city, state=state, houseNumber=full_address
        )

        if 'q31_uploadProfile' in request.FILES:
            new_user_profile.profile_pic = request.FILES['q31_uploadProfile']
        new_user_profile.otp = 1234
        new_user_profile.save()

        user_id = new_user.id

        return HttpResponseRedirect(reverse('cc:otp_page', kwargs={'id': user_id}))
...
def otp_verify(request, id):
    ...
    if request.method == 'POST':
        ...
        if user_otp.otp == otp:
            ...
            UserProfile.objects.filter(user=user).update(active_account=True)
            user.save()
            print("Activated")
            ...
            if request.user == current_user:
                return HttpResponseRedirect(reverse('cc:homepage'))
            return HttpResponseRedirect(reverse('cc:LoginView'))

        else:
            print("not same")

            message = "Verification failed, please enter the correct OTP"
...
def newCommunity(request):
    ...
    if request.method == 'POST':
        ...
        if community_form.is_valid():
            ...
            community.save(), UserProfile.objects.all().filter(
                user=request.user).update(joined_in_community=True)
...
def busReg(request):
    ...
    if request.method == 'POST':
        ...
        if busreg_form.is_valid():
            ...
            return HttpResponseRedirect(reverse('cc:hotelhome'))
# if 'business_logo' in request.FILES:
...
def FoodItemDetailView(request, id):
    ...
    if request.method == 'POST':
        new_order = RestaurantOrders(user=request.user, hno=request.user.userprofile.houseNumber,
                                     area=request.user.userprofile.Area_Name, payment='Done', price=food_detail.price_per_unit, item=food_detail)
        new_order.save()
        ordered = True
...
def detail(request, id):
    ...
    if detail_object.category == 'Food':
        restt = detail_object.id
        item_names = RestaurantItems.objects.all().filter(
            hotel=restt, available_today=True)
        if request.method == 'POST':
...
def CommunityPostHomeView(request, id):
    ...
    cps = CommunityPosts.objects.all().filter(
        community=community_s).order_by('-id')
...
def CommunityPostFormView(request, id):
    ...
    if request.method == 'POST':
        ...
        # if 'profile_pic' in request.FILES:
        #             profile.profile_pic = request.FILES['profile_pic']
        newPost = CommunityPosts(
            title=title, text_body=body, community=community, author=request.user)
        if 'post_pic' in request.FILES:
            newPost.attachment_pic = request.FILES['post_pic']
        newPost.save()
...
def ProcessOrderView(request, id):
    ...
    if request.method == 'POST':
        ...
        new_confirmed_order = ConfirmedOrders(
...
def MyOrdersView(request):
    ...
    my_completed_orders = ConfirmedOrders.objects.all().filter(
...
def group_detail(request, id):
    ...
    if request.method == 'POST':
        ...
        print(request.user)
        form_model = Comment(name=request.user, body=request.POST.get(
            'body'), post=GroupPosts.objects.get(id=int(request.POST.get('id'))))
        form_model.save()
    ...
    context = {'gps': gps, 'id': group_s.id,
               'detail_object': detail_object, 'comment_form': comment_form}
...
def GroupPostFormView(request, id):
    ...
    if request.method == 'POST':
        ...
        newPost = GroupPosts(title=title, text_body=body,
...
def GroupApprovalsView(request, id):
    ...
    group_requests = GroupRequests.objects.all().filter(
        groupname=group).order_by('-timestamp')
    params = {'group_requests': group_requests}
...
def groupjoin(request, id, group_id, member):
    ...
    group_add.people.add(member), UserProfile.objects.all().filter(
        user=member).update(joined_in_group=True)
